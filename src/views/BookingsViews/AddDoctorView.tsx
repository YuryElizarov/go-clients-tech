...
function AddDoctorView() {
    ...
    return (
        <Wrapper>
            <Header>
                <HeadingStyled as="h3">{t("add_doctor.title")}</HeadingStyled>
            </Header>
            <Content>
                <UserInfo>
                    <UserPhoto>
                    ...
                    <Inputs>
                        <Input value={data.title} onChange={val => onChange('title', val)} id="title"
                               label={t("add_doctor.label.title") as string}
                        />
                        <InputsRow>
                            ...
                            <MultiselectInput value={data.location} placeholder=''
                                              ...
                                              label={t("add_doctor.label.address") as string}/>
                        </InputsRow>
                    </Inputs>
                </UserInfo>
                ...
                <ServiceHidden>
                    <ToggleBlock>
                        <LabelBlock><ClinicIcon/>
                            <div>{t("add_doctor.hidden") as string} <span>{t("add_doctor.after") as string}</span></div>
                        </LabelBlock>
                    </ToggleBlock>
                ...
                </ServiceHidden>
                {
                    data.isHidden && <Services>
                        <Service>
                            <ServiceLabel><MessageIcon/>{t("add_doctor.services.messages")}</ServiceLabel>
                            <Toggle isActive={data.isMessages} onToggle={() => onChange('isMessages', !data.isMessages)}/>
                        </Service>
                        <Service>
                            <ServiceLabel><BookingIcon/>{t("add_doctor.services.online_booking")}</ServiceLabel>
                            <Toggle isActive={data.isOnlineBooking} onToggle={() => onChange('isOnlineBooking', !data.isOnlineBooking)}/>
                        </Service>
                        <Service>
                            <ServiceLabel><PatientIcon/>{t("add_doctor.services.patients")}</ServiceLabel>
                            <Toggle isActive={data.isPatients} onToggle={() => onChange('isPatients', !data.isPatients)}/>
                        </Service>
                        <Service>
                            <ServiceLabel><WalletIcon/>{t("add_doctor.services.payments")}</ServiceLabel>
                ...
                <ButtonBlock>
                    <FooterButton variant={EButtonVariants.Text}>{t("add_doctor.discard")}</FooterButton>
                    <FooterButton variant={EButtonVariants.Default}>{t("add_doctor.save")}</FooterButton>
                </ButtonBlock>
            </Content>
        </Wrapper>
    );
