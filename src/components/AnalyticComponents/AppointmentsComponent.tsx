...
const dataset = labels.map(() => faker.datatype.number({min: 1, max: 20000}))
...
function AppointmentsComponent() {
    ...
    return (
        <Wrapper>
            <Header>
                <TitleStyled>{t('analytic.appointments.title')}</TitleStyled>
                <Buttons>
                    <Tabs>
                        {
                            tabs.map((item, id) =>
                                <Tab key={`Tab-${id}`} isSelected={id === selectedTab}
                                    ...
                                    {t(`analytic.appointments.tabs.${item}`)}
                                </Tab>)
                        }
                    </Tabs>
                    <ButtonStyled
                        variant={EButtonVariants.Gray}><Download/>{t('analytic.appointments.button')}
                    </ButtonStyled>
                </Buttons>
            </Header>
            <WrapGraph>
            ...
            <List>
                {
                    mockStatistic.map((item, id) => <StatisticItem key={`appointments-${id}`} {...item}
                                                                   title={t(`analytic.appointments.statistic.${item.title}`)}/>)
                }
            </List>
        </Wrapper>
    );
