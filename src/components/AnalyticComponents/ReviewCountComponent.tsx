...
function ReviewCountComponent() {
    const {t} = useTranslation()
    return (
        <Wrapper>
            <Header>
                <TitleStyled>{t('analytic.reviews_count.title')}</TitleStyled>
                <ButtonStyled
                    variant={EButtonVariants.Gray}><Download/>{t('analytic.reviews_count.button')}
                </ButtonStyled>
            </Header>
            <WrapGraph>
            ...
            <List>
                {
                    mockStatistic.map((item, id) => <StatisticItem key={`analytic-${id}`} {...item}
                                                                   title={t(`analytic.reviews_count.statistic.${item.title}`)}/>)
                }
            </List>
        </Wrapper>
    );
