...
function AddServiceModal() {
    ...
    if (!modals[EModals.AddService]) return null;
    return (
        <Modal modal={EModals.AddService} title={t("booking.add_service.title")}>
            <Content>
                <Input value={data.title} onChange={val => onChange('title', val)} id="title"
                       label={t("booking.add_service.label.title") as string}/>
                <InputsRow>
                    <DropdownInput value={data.delay} placeholder='' onSelect={val => onChange('delay', val)}
                                   options={opts} label={t("booking.add_service.label.delay" as string)}/>
            ...
            <Footer>
                <ToggleBlock>
                    <LabelBlock><EyeOffIcon/> <div>{t("booking.add_service.hidden") as string} <span>{t("booking.add_service.after") as string}</span></div></LabelBlock>
                    <Toggle isActive={data.isHidden} onToggle={() => onChange('isHidden', !data.isHidden)}/>
                </ToggleBlock>
                <FooterButton variant={EButtonVariants.Default}>{t("booking.add_service.public") as string}</FooterButton>
            </Footer>
        </Modal>
    );
}
